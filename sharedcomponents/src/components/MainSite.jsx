...
export default function MainSite({ scrollTarget, clearScrollTarget }) {

     useEffect(() => {
    // If a scroll target was passed down from the header
    if (scrollTarget) {
      const element = document.getElementById(scrollTarget);
      if (element) {
        element.scrollIntoView({ behavior: 'smooth' });
      }
      // Reset the target in parent state so it doesn't re-scroll on accidental updates
      clearScrollTarget();
    }
